...
%   add by choin0207 2021/0507 for parallel processing num
%   ======================================================================
logit;
...
parallel_num = getparm('parallel_num',1); % add by choin0207 2021/0507 for parallel processing num
    ...
    elseif parallel_num > 1
        ...
        % crude bug fix is to drop this patch. It needs fixing in future...
            if j_min<1 || i_min<1
                ...
                ph_filt = zeros(n_win,n_win,n_ifg); % add parfor by choin0207 2021/05/07
                for i_ifg=1:n_ifg
                    ph_filt(:,:,i_ifg)=clap_filt_patch(ph_bit(:,:,i_ifg),clap_alpha,clap_beta,pm.low_pass);
                end
            ...
            if i/10000==floor(i/10000)
                %logit(sprintf('%d patches re-estimated',i))
                fprintf('%s: %d patches re-estimated\n',upper(mfilename),i);
            end
        end
    end
