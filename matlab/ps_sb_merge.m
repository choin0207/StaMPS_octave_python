...
sb_wd1=fullfile(ps_wd(1:i+13), 'SMALL_BASELINES');
sb_wd=fullfile(sb_wd1, ps_wd(i+14:end));
merged_wd1=fullfile(ps_wd(1:i+13), 'MERGED');
merged_wd=fullfile(merged_wd1,ps_wd(i+14:end));

if ~exist(merged_wd)
    mkdir(merged_wd)
end
if ~exist(fullfile(merged_wd,'patch_noover.in')) & exist(fullfile(sb_wd,'patch_noover.in'))
    copyfile(fullfile(sb_wd,'patch_noover.in'),...
        fullfile(merged_wd,'patch_noover.in'));
    %aa=['!cp ',sb_wd,'/patch_noover.in ',merged_wd];
    %eval(aa)
end
if ~exist(fullfile(merged_wd1,'parms.mat'))
    copyfile(fullfile(sb_wd1,'parms.mat'),...
        fullfile(merged_wd1,'parms.mat'));
    %aa=['!cp ',sb_wd1,'/parms.mat ',merged_wd1,'/parms.mat']; 
    %eval(aa)
end

psver=2;
save(fullfile(merged_wd,'psver'),'psver')
...
save(fullfile(merged_wd,'merge2'),'psu_ix','psu_coh','psnu_ix','sbnu_ix','psnu_coh','sbnu_coh','G')
...
save(fullfile(merged_wd,'ps2'),'-struct','ps')
...
save(fullfile(merged_wd,'pm2'),'-struct','pm')
clear pm pmps pmsb
...
save(fullfile(merged_wd,'rc2'),'-struct','rc')
clear rc rcps rcsb
...
save(fullfile(merged_wd,'bp2'),'-struct','bp')
clear bp bpps bpsb


hgtps=load(fullfile(ps_wd,'hgt2.mat'));
hgtsb=load(fullfile(sb_wd,'hgt2.mat'));
hgt=hgtsb;
hgt.hgt=[hgtps.hgt(psu_ix,:);hgt.hgt];
hgt.hgt=hgt.hgt(sort_ix,:);
save(fullfile(merged_wd,'hgt2'),'-struct','hgt')
clear hgt hgtps hgtsb

laps=load(fullfile(ps_wd,'/la2.mat'));
lasb=load(fullfile(sb_wd,'/la2.mat'));
la=lasb;
la.la=[laps.la(psu_ix,:);la.la];
la.la=la.la(sort_ix,:);
save(fullfile(merged_wd,'la2'),'-struct','la')
clear la laps lasb
logit(1);
