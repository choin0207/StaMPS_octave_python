...
% ── Octave parallel compatibility ────────────────────────────────────────────
% parfor requires the 'parallel' package in Octave.
% Install with: sudo apt install octave-parallel
% If not available, parfor falls back to serial for-loop automatically.
...
%               angle file
% add "parallel_num" for parallel processing by choin0207 2021/05/07
%   ==============================================================
logit;
...
parallel_num = getparm('parallel_num',1);
...
while loop_end_sw==0
  %if step_number==1     % check in case restarting and step 1 already completed
    ...
    elseif parallel_num >1
        if isempty(gcp('nocreate')) && ~exist('poolobj','var')
            poolobj = parpool(parallel_num); % set parallel numbers
        end
        
        % ======== parallel loop 1 ===========
        %{
        tic
        ind_grid_ij = sub2ind([n_i n_j], grid_ij(:,1), grid_ij(:,2));
        ind_grid_ij_uniq = unique(ind_grid_ij); % find uniq index of all ps
        tmp_grid = zeros(length(ind_grid_ij_uniq),n_ifg); % tmp matrix for
        ph_grid = reshape(ph_grid,n_i*n_j,n_ifg);
        fprintf('=== sart loop 1 ====\n')
        for i = 1:length(ind_grid_ij_uniq)
            tmp_grid(i, :) = sum(ph_weight(ind_grid_ij==ind_grid_ij_uniq(i),:),1);           
        end
        fprintf('==== end loop 1 ====\n')
        ph_grid(ind_grid_ij_uniq,:) = tmp_grid;
        ph_grid = reshape(ph_grid, n_i, n_j, n_ifg);
        toc
        pause
        %}
        ...
        % ======== parallel loop 3 ===========
        %
        ph_filt = reshape(ph_filt, n_i*n_j, n_ifg);
        ind_ij = sub2ind([n_i n_j],grid_ij(:,1),grid_ij(:,2));
        parfor i = 1:n_ps
            ph_patch(i,:) = squeeze(ph_filt(ind_ij(i),:));
        end
        ph_filt = reshape(ph_filt, n_i, n_j, n_ifg);
        %}
        % ======================================
...
if exist('poolobj','var')||~isempty(gcp('nocreate'))
    delete(gcp);
end
