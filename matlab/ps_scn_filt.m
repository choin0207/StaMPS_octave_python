...
fprintf(sprintf('   Number of points per ifg: %d\n',n_ps))

nodename=['scnfilt.1.node'];
fid=fopen(nodename,'w');
fprintf(fid,'%d 2 0 0\n',n_ps);
fprintf(fid,'%d %f %f\n',[1:n_ps; ps.xy(:,2)'; ps.xy(:,3)']);
%for i=1:n_ps
...
fprintf('=== triangle calculating ====\n')
arch=computer('arch'); % check system type
if strcmpi(arch(1:3),'win')
    % use triangle install in WSL 
    system('wsl triangle -e scnfilt.1.node > triangle_scn.log');
else
