function Data = StaMPSimgRead(filename,bkformat,machinefmt)
%  BKFORMAT:
...
% EX:dem=StaMPSimgRead('INSAR_20210205/geo/elevation_dem.rdc','float32','b');dem(dem(:)<0)=nan;
% EX:lon=StaMPSimgRead('INSAR_20210205/geo/20210205.lon','float32','b');
% EX:lat=StaMPSimgRead('INSAR_20210205/geo/20210205.lat','float32','b');
% EX:slc=StaMPSimgRead('INSAR_20210205/rslc/20210205.rslc','cpxfloat32','b');imagesc(10*log10(abs(slc)));
% EX:intf=StaMPSimgRead('INSAR_20210205/diff0/20210205_20200106.diff','cpxfloat32','b');
%    imagesc(atand(imag(intf)./real(intf)));
parfile=[filename '.par'];
[p1,p2] = textread(parfile,'%s %s','delimiter',':');
for i = 1:length(p1)
    switch p1{i}
        case 'azimuth_lines'
            nlines = str2double(p2{i});
            break;
        case 'nlines'
            nlines = str2double(p2{i});
            break;
...
Data = freadbkj(filename, nlines, bkformat, machinefmt);
