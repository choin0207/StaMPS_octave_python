...
if use_small_baselines==0
    ...
    if strcmpi(arch(1:3),'win')
        if exist([replace(meanvname,'/','\'), '.mat'],'file')
            fprintf(['windows system del ',meanvname, '.mat\n']);
            evalcmd=['!del ',replace(meanvname,'/','\'), '.mat'];
            eval(evalcmd);
        end
    else
        if exist([meanvname, '.mat'], 'file')
            fprintf(['linux or OS X system rm ',meanvname, '.mat\n']);
