...
masterdayname=['master_day.1.in'];  % YYYYMMDD for ISCE 2.3
referencedayname=['reference_day.1.in']; % YYYYMMDD for ISCE > 2.3
...
if ~exist(masterdayname,'file')&~exist(referencedayname,'file')
    masterdayname= ['../',masterdayname];
	referencedayname= ['../',referencedayname];
end
if exist(masterdayname,'file')
   master_day=load(masterdayname);
elseif exist(referencedayname,'file')
   master_day=load(referencedayname,'file');
end
...
if exist(calname,'file')
    ...
    for i = 1 : length(calfile)
        ...
        try 
			...
			if strcmpi(aa{end-1},'referencce');
			    aa{end-1}
			    bb=str2num(aa{end-2}(7:14));
			end
        end
